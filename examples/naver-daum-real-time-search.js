function Node(cfg) {
  ...
  module.exports = function (context, params) {
      return new Promise(function(resolve, reject){
            var result = {}
            var hot = []
            getData("https://www.daum.net").then(data => {
                  var keywords = [];
                  var parser = new DomParser();
                  var dom = parser.parseFromString(data);
                  var datas = dom.getElementsByClassName("link_issue")
                  for(var i=0; i < datas.length; i++){
                      if(keywords.indexOf(datas[i].innerHTML) < 0){
                           keywords.push(datas[i].innerHTML)
                      }
                  }
                  result.daum = keywords
  
            getData("https://www.naver.com").then(data => {
                  var keywords = [];
                  var parser = new DomParser();
                  var dom = parser.parseFromString(data);
                  var datas = dom.getElementsByClassName("ah_k")
                  ...
                  for(var i=0; i < loop ; i++){
                      keywords.push(datas[i].innerHTML)
                      if( result.daum.indexOf(datas[i].innerHTML) >= 0) {
                            hot.push(datas[i].innerHTML)
                      }
                  }
                  result.naver = keywords
                  result.hot = hot
                  resolve( result)
             }).catch(error => {
                  result.naver = {}
